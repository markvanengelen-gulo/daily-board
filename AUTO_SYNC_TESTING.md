...
**What it does**: Checks GitHub for remote file updates every 5 seconds automatically.
...
**Updates**: Refreshes every 5 seconds to show relative time
...
5. Wait a few seconds for auto-save to complete
6. Open the app on Device B (or Browser Tab 2) with same token
7. Wait 5-10 seconds for auto-sync to detect changes
8. Verify the change appears on Device B
9. Check "Last sync" indicator shows recent time

**Expected Results**:
- Device B should automatically detect and fetch changes within 5-10 seconds
...
3. Make a change (it will auto-save)
4. Wait a few seconds and observe "Last sync: just now"
5. Wait 2 minutes
...
- Pending changes sync within 5-10 seconds of reconnection
...
6. Bring Device A online (changes auto-save)
7. Bring Device B online
8. Wait 5-10 seconds for auto-sync to detect conflict
9. On Device B, if making new changes, a conflict may be detected

**Expected Results**:
- SHA mismatch detected when trying to save
...
**Steps**:
1. Edit `app.js` and change `intervalMs: 5000` to `intervalMs: 10000`
2. Reload the app
3. Make a change on Device A
4. Verify Device B detects changes within 10-15 seconds
...
2. Reload the app
3. Make a change on Device A (it will auto-save to GitHub)
4. On Device B, wait 60 seconds
5. Verify changes do NOT appear automatically
6. Reload the page on Device B manually
7. Verify changes now appear

**Expected Results**:
- No automatic polling occurs
- Console shows: "[Auto-Sync] Disabled"
- Changes still save to GitHub when made
- Manual page reload fetches latest data
...
- 720 requests per hour = **well within limits**

To reduce API calls:
- Increase `intervalMs` (e.g., 10000 for 10 seconds, or 30000 for 30 seconds)
- Use WebSocket server for real-time sync instead
- Disable auto-sync and rely on page reloads
...
- **Check**: Wait full 5-10 seconds between checks
- **Check**: Remote file actually changed (check GitHub)

**Issue**: Too many API requests
- **Solution**: Increase `intervalMs` to 10000 or higher
